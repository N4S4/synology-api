...
class Certificate(base_api.BaseApi):
    ...
    def upload_cert(self,
        ...
        with open(serv_key, 'rb') as payload_serv_key, open(ser_cert, 'rb') as payload_ser_cert:
            files = {'key': ("server.key", payload_serv_key, 'application/x-x509-ca-cert'),
                     'cert': ("server.crt", payload_ser_cert, 'application/x-x509-ca-cert')}
            if ca_cert:
                with open(ca_cert, 'rb') as payload_ca_cert:
                    files['inter_cert'] = (
                        "ca.crt", payload_ca_cert, 'application/x-x509-ca-cert')
